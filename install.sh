...
deploy_node() {
    if is_master; then
	    ...
	    docker cp "$I_PATH"/secretpad_jar/secretpad.jar root-kuscia-master-secretpad:/app/secretpad.jar
		docker cp "$I_PATH"/secretpad_jar/config/components/secretpad.json root-kuscia-master-secretpad:/app/config/components/secretpad.json
		docker cp "$I_PATH"/secretpad_jar/config/i18n/secretpad.json root-kuscia-master-secretpad:/app/config/i18n/secretpad.json
		docker restart root-kuscia-master-secretpad
		echo "master deploy done"
	elif is_lite; then
		bash "$I_PATH"/deploy_sfpad/deploy_script/lite_deploy.sh "${NODE_ID}" latest
	    docker cp "$I_PATH"/secretpad_jar/secretpad.jar root-kuscia-lite-secretpad-"${NODE_ID}":/app/secretpad.jar
		docker cp "$I_PATH"/secretpad_jar/config/components/secretpad.json root-kuscia-lite-secretpad-"${NODE_ID}":/app/config/components/secretpad.json
		docker cp "$I_PATH"/secretpad_jar/config/i18n/secretpad.json root-kuscia-lite-secretpad-"${NODE_ID}":/app/config/i18n/secretpad.json
		docker restart root-kuscia-lite-secretpad-"${NODE_ID}"
		echo "lite deploy done"
	else
        echo "Error: Unknown deployment mode"
        return 1
    fi
}
